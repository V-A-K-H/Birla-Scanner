...
const QrScanner = () => {
  ...
  const handleScan = async scanData => {
    if (scanData && scanData !== '') {
      if (
        new Date().getTime() -
          new Date(scanData.text.split('$$')[0]).getTime() >=
        7000
      ) {
        ...
        <>
          <select
            ...
            <option value={'environment'}>
              <span className="cameramodeoption">Back Camera</span>
            </option>
            <option value={'user'}>
              <span className="cameramodeoption">Front Camera</span>
            </option>
          </select>
          ...
          <QrReader
            ...
            constraints={{
              video: {
                facingMode:
                  selected === 'user' ? 'user' : {exact: 'environment'},
              },
              audio: false,
            }}
            delay={1000}
